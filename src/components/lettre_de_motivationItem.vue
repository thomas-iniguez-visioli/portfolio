...
Object.keys(document.getElementsByTagName("a")).map((ite)=>{
  ...
  if(item.id){
    ...
    log.info(item.href.includes("lettre_de_motivation"))

      item.href=window.location.href+"lettre_de_motivation/"+item.textContent.split("/")[item.textContent.split("/").length-1]
